...
def AccommodationProcess(execution_date, **kwargs):
    ...
    file_name = f"{date_str}-AccommodationItem.csv"
...
def DescriplinesProcess(execution_date, **kwargs):
    ...
    descriplines_df = pd.DataFrame({
        ...
        'checkin_start': data['checkin'].apply(lambda x: extract_checkin_checkout_times(str(x))[0]),
        'checkin_end': data['checkin'].apply(lambda x: extract_checkin_checkout_times(str(x))[1]),
        'checkout_start': data['checkout'].apply(lambda x: extract_checkin_checkout_times(str(x))[0]),
        'checkout_end': data['checkout'].apply(lambda x: extract_checkin_checkout_times(str(x))[1])
    })
...
def RoomProcess(execution_date, **kwargs):
    #execution_date = datetime.datetime.strptime(execution_date, '%Y-%m-%d')
    date_str = execution_date.strftime('%Y-%m-%d')  
    file_name = f"{date_str}-RoomPriceItem.csv"
    file_path = f'./data/{file_name}'
...
def BedPriceProcess(execution_date, **kwargs):
    ...
    data['price'] = data['price'].fillna(data['price'].mean()).astype('int64')
...
def load_to_db(**kwargs):
    ...
    accommodation_data = ti.xcom_pull(task_ids='process_accommodation_data', key='accommodation_data') 
    accommodation_df = pd.DataFrame(accommodation_data)
    accommodation_df.to_sql('Accommodation', engine, if_exists='append', index=False)
    print("Data loaded successfully to Accomodation table.")

    descriplines_data = ti.xcom_pull(task_ids='process_descriplines_data', key='descriplines_data') 
    descriplines_df = pd.DataFrame(descriplines_data)
    descriplines_df.to_sql('Descriplines', engine, if_exists='append', index=False)
    print("Data loaded successfully to Descriplines table.")

    room_data = ti.xcom_pull(task_ids='process_room_data', key='room_data') 
    room_df = pd.DataFrame(room_data)
    room_df.to_sql('Room', engine, if_exists='append', index=False)
    print("Data loaded successfully to Room table.")

    bed_price_data = ti.xcom_pull(task_ids='process_bed_price_data', key='bed_price_data') 
    bed_price_df = pd.DataFrame(bed_price_data)
    bed_price_df.to_sql('Bed_price', engine, if_exists='append', index=False)
    print("Data loaded successfully to Bed_price table.")'''
